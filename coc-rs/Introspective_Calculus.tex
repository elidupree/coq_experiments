...
\begin{document}
  ...
  \begin{align*}
    Atom &\to \atomemptyset \mid \atomunion \mid \atomall \mid \atomimplies \mid \const \mid \fuse\\
    F &\to Atom \mid FF \\
  \end{align*}
  ...
  [Now, as for inference, we depart a little from the usual convention]
  
  For $(A \to B)$ to be \emph{true in IC}, means that $A$ and $B$ are sets of inference rules, and $B$ is derivable from $A$, using only the basic properties of how inference rules work.
  
  For $(A \to B)$ to be \emph{an inference rule of IC}, means that $A$ and $B$ are sets of statements, and if the statements $A$ are true in IC, then $B$ must also be true in IC.
  
  As we shall see, all true statements of IC are also derivable inferences of IC, but the converse is not true: some inference rules of IC are not true statements, because they only work for IC in particular, rather than being universal truths about how inference works.
