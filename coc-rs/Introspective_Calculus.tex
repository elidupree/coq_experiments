...
\begin{document}
  ...
  \subsection{[Motivation: The paradox / What is truth?]}
  
  We will want to be able to express the following:
  \begin{itemize}
    \item that some formulas are \emph{true};
    \item that some formulas are \emph{logically equivalent} to each other, $A \equiv B$ (read as "A equals B");
    \item universal quantification, saying that all formulas have some property.
  \end{itemize}
  
  The introspection principle says we should be able to write \emph{formulas} that express these concepts.
  With these tools, we can immediately define absurdity and negation: $\bot$ (read as "false") is a formula that says "all formulas are true".
  (Clearly we don't want this to be true.)
  And we can express "not P" as the formula $P \equiv \bot$. 
  
  If we can do this, then we can construct a version of the liar paradox, where:
  
  \newcommand{\name}[1]{\operatorname{\texttt{#1}}}
  \begin{equation*}
    \name{liar}_0 \equiv (\name{liar}_0 \equiv \bot)\\
  \end{equation*}
  ...
  If it were true, we could argue as follows: $\name{liar}_0$ is true; therefore the equivalent statement, $(\name{liar}_0 \equiv \bot)$, is true; therefore $\name{liar}_0$ and $\bot$ are equivalent, so since $\name{liar}_0$ is true, $\bot$ is also true.
  
  Naïvely, the above argument suggests that $\name{liar}_0$ is equivalent to $\bot$.
  But that's the same as saying $\name{liar}_0$ is true.
  So we must say they are \emph{not} equivalent, or we immediately succumb to the paradox.
  On the other hand, the above argument does say \emph{something}, so the introspection principal asks us to be able to express that: We will say $\name{liar}_0 \leadsto \bot$ (read as "liar zero \emph{leads to} false") to express the notion: "\emph{if} $\name{liar}_0$ were true, then we could reach $\bot$ by argument".
  
  This immediately allows us to construct a second version of the paradox:
  
  \begin{equation*}
    \name{liar}_1 \equiv (\name{liar}_1 \leadsto \bot)\\
  \end{equation*}
  
  At this point, it is tempting to argue: "Suppose $\name{liar}_1$ is true; then $(\name{liar}_1 \leadsto \bot)$ is true; this means there exists an argument taking $\name{liar}_1$ to $\bot$, so simply apply that argument and obtain $\bot$."
  But this argument can't be valid, or it would prove $\name{liar}_1$.
   We must say that you can't automatically apply an argument just because of a hypothetical belief that one exists – because that belief might not be true.
   
  "But," you say, "if it were \emph{actually true}, and it concluded that an argument existed, then that argument \emph{would} exist, and you could use it!"
  We will capture this in a notion of "arguments that are allowed to apply $\leadsto$ arguments", which we will express as $\stackrel{1}{\leadsto}$ (read as "A leads to B at level 1"), to hint at the endlessness of the task.
  We can now construct a third liar paradox, $\name{liar}_2 \equiv (\name{liar}_2 \stackrel{1}{\leadsto} \bot)$, which only leads to $\bot$ at level 2 (expressed as $\stackrel{2}{\leadsto}$, an argument that is allowed to apply $\stackrel{1}{\leadsto}$ arguments).
  This will go on forever; in fact, we may construct an "argument level" for every ordinal number, not just the natural numbers.
  
  "But," whispers the introspection principle, "surely we still believe that it's impossible for any of these paradoxical formulas to be true. We should be able to express the property of impossibility, $\stackrel{\infty}{\leadsto})$"
  
  [well, I haven't solved that, but maybe it isn't coherent in the metatheory either, because all ordinals]
  ...
  \begin{align*}
    \label{subst}\tag{substitute whole formula}
  ...
  We're going to be much more ambitious with this relation than usual: By the end of our definitions, all logically-equivalent formulas will be definitionally equal (in particular, all true formulas are equal to each other), and extensionally equal functions will also be equal.
  ...
  \begin{itemize}
    ...
    \item Next, we define $\name{All}$, the function where $\name{All} P$ means $\forall x, Px$. This can be implemented as $(\atomequals (\const \top))$, because $(\const \top))$ is the canonical "function that always returns a true proposition".
    \item Now, the general case of $\forall x, B$ can be defined as $\name{All} (\nameabst{x} B)$.
  \end{itemize}
  ...
  \begin{align*}
    %(\forall x, \singleton{\emptyset \vdash \singleton{x \in \singleton{x}}})\\
  ...
  \begin{align*}
    ...
    %\globalrule{}{(\emptyset \infrule \emptyset) \in R} &&
%    \globalrule{(A \infrule B) \in R \presep (B \infrule C) \in R}{(A \infrule C) \in R} &&
%    \globalrule{external.\forall x, ((Ax \infrule Bx) \in R)}{((\forall x, Ax) \infrule (\forall x, Bx)) \in R}
  \end{align*}

%  \begin{align*}
%    \globalrule{(A \infrule B) \in R \presep }{(A \union B) \in R} &&
%    \globalrule{\forall x, (Ax \in R)}{(\forall x, Ax) \in R}
%  \end{align*}
%
%  \begin{align*}
%    \globalrule{A \in R \presep B \in R}{(A \union B) \in R} &&
%    \globalrule{\forall x, (Ax \in R)}{(\forall x, Ax) \in R}
%  \end{align*}
...
%
%  \begin{align*}
%    \globalrule{}{R \concludes \emptyset} &&
%    \globalrule{R \concludes A \presep (A \infrule B) \in R}{R \concludes \singleton{B}} &&
%    %\globalrule{R \concludes A \presep X \in A}{R \concludes \singleton{X}} &&
%    \globalrule{R \concludes A \presep R \concludes B}{R \concludes (A \union B)} &&
%    \globalrule{ext.\forall b, (R \concludes Ab)}{R \concludes \forall b, Ab}
%  \end{align*}
  ...
  \begin{align*}
    ...
    % &\vdash ((\emptyset \infrule B) \concludes B) \\
    \tag{rules do what they say}
    ((A \infrule B) \in R), (R \concludes A) &\vdash (R \concludes B) \\
    % (R \concludes A), ((R \union (\emptyset \infrule A)) \concludes B) &\vdash (R \concludes B) \\
  \end{align*}
  \fi  
