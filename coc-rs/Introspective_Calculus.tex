...
%  \newcommand{\usage}{\mathcal{V}}
  ...
  \begin{align*}
    ...
    \abst{A}{B}{C} &:= (((\mathrm{substitute\_in}\ A) B) C)\\
    \usage &:= \mathrm{usage}\\
  ...
  This is defined as $(\abst{B'}{0})$, where $B'$ is $B$ with the following changes:
  \begin{itemize}
    \item Replace all instances of $A$ with the formula $\usage$.
    \item Insert the minimum collection of pops such that, according to the substitution axioms below, branches of $\abst{B'}{\lzero}{C}$ will reach every usage of $A$, and every branch that doesn't will end in one of these pops.
  \end{itemize}
...
%    \inferrule{A \equals B}{B \equals A}\\
    ...
    (A &\equals B) \equals (\nameabst{P} PA \equals PB)\\
  \end{align*}
  Substitution:
%  \setlength{\jot}{0.1em}
  \begin{align*}
    (\abst{\usage}{\lzero}{C}) &\equals C\\
    (\abst{\pop{A}}{\lzero}{C}) &\equals A\\
    (\abst{\pop{A}}{\lsucc{n}}{C}) &\equals \pop{(\abst{A}{n}C)}\\
    (\abst{\abst{A}{0}}{n}{C}) &\equals \abst{(\abst{A}{\lsucc{n}}{C})}{0}\\
    (\abst{(AB)}{n}{C}) &\equals (\abst{A}{n}{C})(\abst{B}{n}{C})\\
  \end{align*}
  Abstraction:
  \begin{align*}
%    \tag{truth of foralls}
%    A &\implies{\wfz} (\abst{A})\\
    ...
    (\abst{A}{0}) \implies{\lzero} \abst{A}{0}B\\
  \end{align*}
  ...
  \begin{align*}
    ...
    \mathrm{proof\_induction} \equals \nameabst{P} (P \equals\,&\nameabst{R}\\
      &\nameabst{n}\\
