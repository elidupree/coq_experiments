...
\begin{document}
  ...
  \begin{align*}
    Atom &:= \const \mid \fuse \mid \atomimplies \mid \atomall\\
    F &:= Atom \mid FF \\
  \end{align*}
  ...
  $\equiv$ (read as "equals") is definitional equality. The notation $A \equiv B$ ?????. Equal formulas can be substituted for each other at any position within a larger formula:
  \begin{align*}
  ...
  \begin{align*}
    ...
    (\all A)\mathbb{T} &\equiv (\forall A, \mathbb{T}A)\\
  \end{align*}
  \begin{align*}
  ...
  \begin{align*}
    ...
    IC_1 &:= \name{FullyInternally}(IC_0 \wedge \name{Theorems} IC_0)\\
