...
\begin{document}
  ...
  \begin{align*}
    Atom &:= \const \mid \fuse \mid \atomemptyset \mid \atomunion \mid \atomall \mid \atomequals \mid \atominfrule \mid \atomconcludes\\
    F &:= Atom \mid FF \\
  \end{align*}
  ...
  All propositions [etc.] will be represented by nested applications ($F$ above) of these elementary functions.
  ...
  \begin{align*}
    ...
    \all A &:= \atomall A & (\text{right-associative, binds weaker than function application})\\
  ...
  We are going to define sets of inference rules, which act on sets of beliefs. [So we need to define sets] We believe the order doesn't matter:
  ...
  $\equals$ (read as "equals") is definitional equality, the relation between things that we see fit to make indistinguishable. Equal formulas can be substituted for each other at any position within a larger formula:
  ...
  \begin{align*}
    ...
    &\vdash A \equals \all (\const A) \\
    &\vdash (\all A) \equals \all (\fuse (\const \atomall) A)\\
    &\vdash \forall b, Ab \equals \forall c, \forall d, A(cd)\\
  \end{align*}
...
Our remaining task is to express the proposition "[the rules set $R$ reaches the conclusion $C$, $R \concludes C$]".
  ...
  [we believe that the above rules allow you to prove all interesting mathematical statements, but
  Also:
  \begin{align*}
