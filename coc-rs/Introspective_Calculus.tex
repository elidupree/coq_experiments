...
\begin{document}
  ...
  \begin{align*}
    Atom &\to \atomemptyset \mid \atomsingleton \mid \atomunion \mid \atomall \mid \atomin \mid \const \mid \fuse \mid \atominfrule \mid \atomconcludes\\
    F &\to Atom \mid FF \\
  \end{align*}
  ...
  We'll need to describe sets of inference rules, which relate sets of propositions. That means we need to define \emph{sets}. To represent a set as a formula, we will express it as a union of singleton elements: The formula $\singleton{A \infrule B} \union \singleton{C \infrule D}$ represents the set that contains two inference rules, $(A \infrule B)$ and $(C \infrule D)$. To use these sets, we'll need inference rules to define the set membership relation, $\in$ (read as "in"):
...
%  \end{align*}
  ...
  (along with 1 internal rule $(\emptyset \infrule \emptyset)$ that we didn't feel the need to specify in the metatheory)

  And the convention is that $(\emptyset \infrule A) \in R$ means "$R$ believes/concludes $A$"
  ...
  [start by proving the fairly simple statement $(\emptyset \infrule \emptyset) \in (\singleton{\emptyset \infrule \emptyset} \union a)$
